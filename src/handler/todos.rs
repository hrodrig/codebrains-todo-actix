...
async fn update_todo(
    ...
    let todo: Option<Todo> = state.todo_repository.update_todo(id.into_inner(), new_todo).await;
    Ok(web::Json(todo))
}
...
async fn delete_todo(
    ...
    let todo: DeleteResult = state.todo_repository.delete_todo_by_id(id.into_inner()).await;
    Ok(web::Json(json!({
        "message": "Todo deleted successfully",
        "deleted": todo.rows_affected
    })))
}
